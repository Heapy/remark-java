...
import by.heap.remark.util.TestUtils;
...
public class TextCleanerTest {
    ...
    public void testCleanInlineCodeSimple() throws Exception {
        assertEquals("`hello &  > world`", BASIC.cleanInlineCode("hello &amp; \n&gt; world"));
    }

    @Test
    public void testCleanInlineCodeLeadingTick() throws Exception {
        assertEquals("`` `tick``", BASIC.cleanInlineCode("`tick"));
    }
    ...
    public void testCleanInlineCodeInlineTick() throws Exception {
        assertEquals("``ti`ck``", BASIC.cleanInlineCode("ti`ck"));
    }

    @Test
    public void testCleanInlineCodeLotsOfTicks() throws Exception {
        assertEquals("```` ``t```i`ck` ````", BASIC.cleanInlineCode("``t`&#96;`i`ck`"));
    }

    /** Disabled because this is too dependent on the computer
     @Test public void testReplacementPerformance() throws Exception {
     ...
     Assertions.assertTrue("ReplacementPerformance is too slow.", expectedTime > (end-start));
     }
     */
}
